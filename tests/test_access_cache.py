"""Tests for the claim-access TTL cache."""
...
def test_first_call_misses_then_caches(monkeypatch):
    ...
    monkeypatch.setattr(access_cache, "_check_claim_access", fake_check)
    assert access_cache.check_claim_access_cached(None, _user(), "m1", "viewer") is True
    assert access_cache.check_claim_access_cached(None, _user(), "m1", "viewer") is True
...
def test_different_keys_do_not_share_entries(monkeypatch):
    monkeypatch.setattr(access_cache, "_check_claim_access", lambda *a, **k: True)
    access_cache.check_claim_access_cached(None, _user("a"), "m1", "viewer")
    access_cache.check_claim_access_cached(None, _user("b"), "m1", "viewer")
    access_cache.check_claim_access_cached(None, _user("a"), "m2", "viewer")
    access_cache.check_claim_access_cached(None, _user("a"), "m1", "manager")
...
def test_system_admin_bypasses_cache(monkeypatch):
    ...
    access_cache.check_claim_access_cached(None, admin, "m1", "viewer")
    access_cache.check_claim_access_cached(None, admin, "m1", "viewer")
...
def test_invalidate_claim_clears_only_matching_entries(monkeypatch):
    monkeypatch.setattr(access_cache, "_check_claim_access", lambda *a, **k: True)
    access_cache.check_claim_access_cached(None, _user("a"), "m1", "viewer")
    access_cache.check_claim_access_cached(None, _user("b"), "m1", "viewer")
    access_cache.check_claim_access_cached(None, _user("a"), "m2", "viewer")
...
def test_eviction_when_cache_exceeds_max_size(monkeypatch):
    ...
    for i in range(8):
        ...
        access_cache.check_claim_access_cached(None, _user(f"u{i}"), "m", "viewer")
    ...
    access_cache.check_claim_access_cached(None, _user("u9"), "m", "viewer")
