...
from claimos.db import _is_sqlite_url


def test_is_sqlite_url_true_for_sqlite():
    assert _is_sqlite_url("sqlite:///./data/claimos.db") is True
    assert _is_sqlite_url("sqlite+pysqlite:///./data/claimos.db") is True
